...
class ResidualAttentionModel_448input3(nn.Module):
    # for input size 448
    def __init__(self):
        super(ResidualAttentionModel_448input3, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias = False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
...
class ResidualAttentionModel_448concat(nn.Module):
    # for input size 448
    def __init__(self):
        super(ResidualAttentionModel_448concat, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias = False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
    ...
    def forward(self, x,y):
        ...
        out = self.residual_block5(out)
        out = self.residual_block6(out)
        out = self.mpool2(2*out)

        out1 = self.conv1(y)
        out1 = self.mpool1(out1)
        out1 = self.residual_block0(out1)
        out1 = self.attention_module0(out1)
        # print(out.data)
        out1 = self.residual_block1(out1)
        out1 = self.attention_module1(out1)
        out1 = self.residual_block2(out1)
        out1 = self.attention_module2(out1)
        out1 = self.attention_module2_2(out1)
        out1 = self.residual_block3(out1)
        # print(out.data)
        out1 = self.attention_module3(out1)
        out1 = self.attention_module3_2(out1)
        out1 = self.attention_module3_3(out1)
        out1 = self.residual_block4(out1)
        out1 = self.residual_block5(out1)
        out1 = self.residual_block6(out1)
        out1 = self.mpool2(out1)

        out=torch.cat((out,out1),2)
...
class ResidualAttentionModel_448concat3class(nn.Module):
    # for input size 448
    def __init__(self):
        super(ResidualAttentionModel_448concat3class, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias = False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
    ...
    def forward(self, x,y):
        ...
        out = self.residual_block5(out)
        out = self.residual_block6(out)
        out = self.mpool2(self.multp*out)
